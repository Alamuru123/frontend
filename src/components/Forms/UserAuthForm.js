...
const LoginForm = () => {
  ...
  const onLogin = async (data) => {
    ...
    await axios.post('https://res-server-hmno.onrender.com/user/login', data)
      .then(res => {
        setToken(res.data);
        ...
        console.error(error);
        alert('Something went wrongd. Please try again.');
      })
  };

  const onRegister = async (data) => {
    // console.log(JSON.stringify(data, null, 2));
    await axios.post('https://res-server-hmno.onrender.com/user/register', data)
      .then(res => {
        if (res.status === 200) {
